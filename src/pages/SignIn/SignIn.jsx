...
const SignIn = () => {
  ...
  const handleSignIn = (e) => {
    e.preventDefault();
    const form = e.target;
    signInUser(form.email.value, form.password.value)
      .then(() => navigate(from))
  ...
  return (
    <section className="bg-slate-950 min-h-screen flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-4xl flex flex-col lg:flex-row items-center gap-12">
        ...
        >
          <Lottie animationData={signinlottie} loop={true} style={{ width: "280px" }} />
          <h2 className="text-white font-black text-3xl mt-4 text-center">Welcome back</h2>
          <p className="text-slate-500 text-sm mt-2 text-center">Sign in to continue your job search</p>
        </motion.div>
